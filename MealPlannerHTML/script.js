...
function initFirebaseAuth() {
  // Sprawdź czy Firebase SDK jest dostępny
  if (!window.firebaseAuth) {
    // Firebase nie skonfigurowany - aplikacja działa lokalnie
    console.log('ℹ️ Firebase nie skonfigurowany - działamy w trybie lokalnym');
    showSyncDisabled();
  ...
  // NIE pokazuj ekranu logowania automatycznie - aplikacja działa bez logowania
  ...
  if (googleSignInBtn) {
    googleSignInBtn.addEventListener('click', async () => {
      try {
        ...
        const result = await window.firebaseSignIn(auth, provider);
        currentUser = result.user;
        useFirebase = true;
        showSyncEnabled(result.user);
  ...
  // Disable sync (Sign Out)
  if (disableSyncBtn) {
    disableSyncBtn.addEventListener('click', async () => {
      if (confirm('Czy na pewno chcesz wyłączyć synchronizację? Dane pozostaną na tym urządzeniu, ale nie będą się synchronizować.')) {
        try {
  ...
  // Auth state listener - sprawdź czy użytkownik był już zalogowany
  window.firebaseAuthChanged(auth, async (user) => {
    if (user) {
      currentUser = user;
      useFirebase = true;
      showSyncEnabled(user);
      await loadDataFromFirebase();
    } else {
      showSyncDisabled();
      loadDataFromLocalStorage();
...
// Pokaż ekran logowania Firebase (wywoływane przez przycisk)
function showFirebaseLogin() {
  const loginScreen = document.getElementById('loginScreen');
  loginScreen.style.display = 'flex';
}

function showSyncEnabled(user) {
  const syncDisabled = document.getElementById('syncDisabled');
  ...
  const syncUserPhoto = document.getElementById('syncUserPhoto');
  
  if (syncDisabled) syncDisabled.style.display = 'none';
  if (syncEnabled) syncEnabled.style.display = 'block';
  if (syncUserName) syncUserName.textContent = user.displayName || 'Użytkownik';
  if (syncUserEmail) syncUserEmail.textContent = user.email;
  if (syncUserPhoto) syncUserPhoto.src = user.photoURL || 'https://via.placeholder.com/40';
}
...
function showSyncDisabled() {
  ...
  if (syncDisabled) syncDisabled.style.display = 'block';
  if (syncEnabled) syncEnabled.style.display = 'none';
}

function showUserInfo(user) {
  // Usunięte - nie używamy już paska użytkownika na górze
  showSyncEnabled(user);
}
...
async function saveToFirebase(key, data) {
  ...
  try {
    const docRef = window.firestoreDoc(window.firebaseDb, `users/${currentUser.uid}/data`, key);
    await window.firestoreSetDoc(docRef, { data: JSON.stringify(data), updatedAt: new Date().toISOString() });
    console.log('✅ Zapisano do Firebase:', key);
  } catch (error) {
...
async function loadDataFromFirebase() {
  console.log('🔄 Synchronizacja danych z Firebase...');
  
  // Load all data keys
  const keys = [
    'caloriesMichalina', 'caloriesMarcin', 'person1Name', 'person2Name',
    'customDishes', 'savedPlans', 'activities', 'weightHistory', 
    'waterIntake', 'fridgeItems', 'prices', 'trainingSchedule', 
    'garminTrainingData'
  ];
...
function loadDataFromLocalStorage() {
  // Just load from localStorage (already implemented)
  loadSavedCalories();
  loadSavedNames();
  loadCustomDishes();
}
...
function updateSyncStatus(status) {
  // Usunięte - nie używamy już paska statusu
  console.log('Sync:', status);
}

// Wrapper for localStorage that also saves to Firebase
